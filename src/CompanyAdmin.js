import TripCodeManagerBackend from "./TripCodeManager";
...
// EMPTY_TRIP removed as scheduling is obsolete.
...
// // ═══════════════════════════════════════════════════════════
// // BUS FORM — defined OUTSIDE BusManagement to prevent remount
// // ═══════════════════════════════════════════════════════════
// function BusForm({ form, setForm, routes, drivers }) {
//     return (
//         <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>
//             <Field label="Bus Number" required>
//                 <input style={INP} value={form.number} onChange={e => setForm(f => ({ ...f, number: e.target.value }))} placeholder="TN-01-XX-0000" />
//             </Field>
//             <Field label="Model" required>
//                 <input style={INP} value={form.model} onChange={e => setForm(f => ({ ...f, model: e.target.value }))} placeholder="Ashok Leyland" />
//             </Field>
//             <Field label="Capacity (seats)" required>
//                 <input style={INP} type="number" value={form.capacity} onChange={e => setForm(f => ({ ...f, capacity: e.target.value }))} />
//             </Field>
//             <Field label="Fuel Type">
//                 <select style={SEL} value={form.fuel} onChange={e => setForm(f => ({ ...f, fuel: e.target.value }))}>
//                     {["Diesel", "CNG", "Electric", "Petrol"].map(x => <option key={x}>{x}</option>)}
//                 </select>
//             </Field>
//             <Field label="Status">
//                 <select style={SEL} value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
//                     {["active", "idle", "maintenance"].map(x => <option key={x}>{x}</option>)}
//                 </select>
//             </Field>
//             <Field label="Year of Manufacture">
//                 <input style={INP} type="number" value={form.year} onChange={e => setForm(f => ({ ...f, year: e.target.value }))} placeholder="2022" />
//             </Field>
//             <Field label="KM Run">
//                 <input style={INP} type="number" value={form.kmRun} onChange={e => setForm(f => ({ ...f, kmRun: e.target.value }))} />
//             </Field>
//             <Field label="Last Service Date">
//                 <input style={INP} type="date" value={form.lastService} onChange={e => setForm(f => ({ ...f, lastService: e.target.value }))} />
//             </Field>
//             <Field label="Assign Route" span>
//                 <select style={SEL} value={form.routeId || ""} onChange={e => setForm(f => ({ ...f, routeId: e.target.value || null }))}>
//                     <option value="">— No Route —</option>
//                     {routes.map(r => <option key={r.id} value={r.id}>{r.id}: {r.name}</option>)}
//                 </select>
//             </Field>
//             <Field label="Assign Driver" span>
//                 <select style={SEL} value={form.driverId || ""} onChange={e => setForm(f => ({ ...f, driverId: e.target.value || null }))}>
...
//                 <Modal title={modal === "add" ? "🚌 Add New Bus" : "✏ Edit Bus"} onClose={() => setModal(null)}>
//                     {/* ✅ FIX: BusForm is now a stable external component, no cursor jump */}
//                     <BusForm form={form} setForm={setForm} routes={routes} drivers={drivers} />
//                     <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
//                         <button style={B.cancel} onClick={() => setModal(null)}>Cancel</button>
//                         <button style={B.primary} onClick={save}>{modal === "add" ? "Add Bus" : "Save Changes"}</button>
//                     </div>
//                 </Modal>
//             )}
//             {modal === "view" && sel && (
//                 <Modal title="🚌 Bus Details" onClose={() => setModal(null)}>
//                     <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
//                         {[["Bus Number", sel.number], ["Model", sel.model], ["Capacity", sel.capacity + " seats"], ["Fuel", sel.fuel], ["Year", sel.year || "—"], ["KM Run", (sel.kmRun || 0).toLocaleString() + " km"], ["Last Service", sel.lastService || "—"], ["Route", routeName(sel.routeId)], ["Driver", driverName(sel.driverId)], ["Status", sel.status]].map(([k, v]) => (
//                             <div key={k} style={{ background: "#f8fafc", padding: "12px 14px", borderRadius: 10 }}>
//                                 <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 700, textTransform: "uppercase" }}>{k}</div>
...
//             )}
//             {modal === "delete" && sel && (
//                 <ConfirmDel name={sel.number} onConfirm={() => { setBuses(p => p.filter(b => b.id !== sel.id)); setModal(null); showToast("Bus deleted.", "error"); }} onClose={() => setModal(null)} />
//             )}
//         </div>
//     );
// }
...
// // ═══════════════════════════════════════════════════════════
// // DRIVER FORM — defined OUTSIDE DriverManagement
// // ═══════════════════════════════════════════════════════════
// function DriverForm({ form, setForm, buses }) {
//     return (
//         <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>
...
//                 <input style={INP} value={form.license} onChange={e => setForm(f => ({ ...f, license: e.target.value }))} />
//             </Field>
//             <Field label="Experience">
//                 <input style={INP} value={form.experience} onChange={e => setForm(f => ({ ...f, experience: e.target.value }))} placeholder="e.g. 5 yrs" />
//             </Field>
//             <Field label="Status">
//                 <select style={SEL} value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
...
//                 <input style={INP} type="number" min="1" max="5" step="0.1" value={form.rating} onChange={e => setForm(f => ({ ...f, rating: e.target.value }))} />
//             </Field>
//             <Field label="Joining Date">
//                 <input style={INP} type="date" value={form.joinDate} onChange={e => setForm(f => ({ ...f, joinDate: e.target.value }))} />
//             </Field>
//             <Field label="Assign Bus">
//                 <select style={SEL} value={form.busId || ""} onChange={e => setForm(f => ({ ...f, busId: e.target.value || null }))}>
...
//                     {/* ✅ FIX: DriverForm is now a stable external component */}
//                     <DriverForm form={form} setForm={setForm} buses={buses} />
//                     <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
//                         <button style={B.cancel} onClick={() => setModal(null)}>Cancel</button>
//                         <button style={B.primary} onClick={save}>{modal === "add" ? "Add Driver" : "Save Changes"}</button>
//                     </div>
//                 </Modal>
//             )}
//             {modal === "view" && sel && (
//                 <Modal title="👤 Driver Profile" onClose={() => setModal(null)}>
//                     <div style={{ textAlign: "center", marginBottom: 20 }}>
//                         <div style={{ width: 72, height: 72, borderRadius: "50%", background: "linear-gradient(135deg,#4f46e5,#8b5cf6)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, fontWeight: 800, margin: "0 auto 10px" }}>{sel.name[0]}</div>
//                         <div style={{ fontSize: 20, fontWeight: 800, color: "#0f172a" }}>{sel.name}</div>
//                         <div style={{ color: "#f59e0b", fontSize: 18, marginTop: 4 }}>{"★".repeat(Math.floor(sel.rating))}{"☆".repeat(5 - Math.floor(sel.rating))} <span style={{ fontSize: 13, color: "#64748b" }}>{sel.rating}</span></div>
...
//             )}
//             {modal === "delete" && sel && (
//                 <ConfirmDel name={sel.name} onConfirm={() => { setDrivers(p => p.filter(d => d.id !== sel.id)); setModal(null); showToast("Driver deleted.", "error"); }} onClose={() => setModal(null)} />
//             )}
//         </div>
//     );
// }
...
// // ═══════════════════════════════════════════════════════════
// // ROUTE FORM — defined OUTSIDE RouteManagement
// // ═══════════════════════════════════════════════════════════
// function RouteForm({ form, setForm }) {
//     return (
//         <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>
//             <Field label="Route Name" required span>
//                 <input style={INP} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="City Center → Airport" />
//             </Field>
//             <Field label="From" required>
//                 <input style={INP} value={form.from} onChange={e => setForm(f => ({ ...f, from: e.target.value }))} placeholder="Starting point" />
//             </Field>
//             <Field label="To" required>
//                 <input style={INP} value={form.to} onChange={e => setForm(f => ({ ...f, to: e.target.value }))} placeholder="Ending point" />
//             </Field>
//             <Field label="Distance">
//                 <input style={INP} value={form.distance} onChange={e => setForm(f => ({ ...f, distance: e.target.value }))} placeholder="e.g. 25 km" />
//             </Field>
//             <Field label="Duration">
//                 <input style={INP} value={form.duration} onChange={e => setForm(f => ({ ...f, duration: e.target.value }))} placeholder="e.g. 45 min" />
//             </Field>
//             <Field label="Fare (₹)">
//                 <input style={INP} type="number" value={form.fare} onChange={e => setForm(f => ({ ...f, fare: e.target.value }))} />
//             </Field>
//             <Field label="Status">
//                 <select style={SEL} value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
...
//                 </select>
//             </Field>
//             <Field label="Stops (comma-separated)" span>
//                 <input style={INP} value={Array.isArray(form.stops) ? form.stops.join(", ") : form.stops} onChange={e => setForm(f => ({ ...f, stops: e.target.value }))} placeholder="Stop 1, Stop 2, Stop 3" />
//             </Field>
//         </div>
//     );
// }
...
//                     {/* ✅ FIX: RouteForm is now a stable external component */}
//                     <RouteForm form={form} setForm={setForm} />
//                     <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
//                         <button style={B.cancel} onClick={() => setModal(null)}>Cancel</button>
//                         <button style={B.primary} onClick={save}>{modal === "add" ? "Add Route" : "Save Changes"}</button>
//                     </div>
//                 </Modal>
//             )}
//             {modal === "delete" && sel && (
//                 <ConfirmDel name={sel.name} onConfirm={() => { setRoutes(p => p.filter(r => r.id !== sel.id)); setModal(null); showToast("Route deleted.", "error"); }} onClose={() => setModal(null)} />
//             )}
//         </div>
//     );
// }
...
// // ═══════════════════════════════════════════════════════════
// // TRIP FORM — defined OUTSIDE TripManagement
// // ═══════════════════════════════════════════════════════════
// function TripForm({ form, setForm, routes, buses, drivers, isEdit }) {
//     return (
//         <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>
//             <Field label="Route" required span>
//                 <select style={SEL} value={form.routeId} onChange={e => setForm(f => ({ ...f, routeId: e.target.value }))}>
//                     <option value="">— Select Route —</option>
//                     {routes.map(r => <option key={r.id} value={r.id}>{r.id}: {r.name}</option>)}
//                 </select>
//             </Field>
//             <Field label="Bus" required>
//                 <select style={SEL} value={form.busId} onChange={e => setForm(f => ({ ...f, busId: e.target.value }))}>
//                     <option value="">— Select Bus —</option>
//                     {buses.map(b => <option key={b.id} value={b.id}>{b.number}</option>)}
//                 </select>
//             </Field>
//             <Field label="Driver" required>
//                 <select style={SEL} value={form.driverId} onChange={e => setForm(f => ({ ...f, driverId: e.target.value }))}>
//                     <option value="">— Select Driver —</option>
...
//                         <input style={INP} type="number" value={form.passengers || 0} onChange={e => setForm(f => ({ ...f, passengers: e.target.value }))} />
//                     </Field>
//                     <Field label="Revenue (₹)">
//                         <input style={INP} type="number" value={form.revenue || 0} onChange={e => setForm(f => ({ ...f, revenue: e.target.value }))} />
//                     </Field>
//                 </>
//             )}
//         </div>
...
//             {(modal === "add" || modal === "edit") && (
//                 <Modal title={modal === "add" ? "📋 Schedule Trip" : "✏ Edit Trip"} onClose={() => setModal(null)} wide>
//                     {/* ✅ FIX: TripForm is now a stable external component */}
//                     <TripForm form={form} setForm={setForm} routes={routes} buses={buses} drivers={drivers} isEdit={modal === "edit"} />
//                     <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
//                         <button style={B.cancel} onClick={() => setModal(null)}>Cancel</button>
//                         <button style={B.primary} onClick={save}>{modal === "add" ? "Schedule Trip" : "Save Changes"}</button>
//                     </div>
//                 </Modal>
//             )}
//             {modal === "delete" && sel && (
//                 <ConfirmDel name={sel.id} onConfirm={() => { setTrips(p => p.filter(t => t.id !== sel.id)); setModal(null); showToast("Trip deleted.", "error"); }} onClose={() => setModal(null)} />
//             )}
//         </div>
//     );
// }
...
//     const [copied, setCopied]       = useState(false);
//     const [search, setSearch]         = useState("");
//     const [searchResult, setSearchResult] = useState(null); // null=not searched, false=not found, obj=found
...
//     // ── District → 2-char abbreviation map ──────────────────
//     // Format: FROM(2)+TO(2)+DATE(2)+MONTH(2)+HOUR(2) = 10 chars
//     // e.g. Tirunelveli→Thoothukudi on 03-Mar 14:xx = "TVTD030314"
//     const DIST_CODE = {
//         "Ariyalur":        "AR",
//         "Chengalpattu":    "CG",
//         "Chennai":         "CH",
...
//     // STRICT 10-char code: FROM(2)+TO(2)+DATE(2)+MONTH(2)+HOUR(2)
//     // Example: Tirunelveli->Thoothukudi on 03-Mar at 14:xx = "TVTD030314"
//     // 10 chars guarantees uniqueness: same route same day different hour = different code
//     function generateCode(fromDistrict, toDistrict) {
//         const now   = new Date();
//         const from2 = DIST_CODE[fromDistrict] || fromDistrict.slice(0,2).toUpperCase();
//         const to2   = DIST_CODE[toDistrict]   || toDistrict.slice(0,2).toUpperCase();
//         const dd    = String(now.getDate()).padStart(2, "0");
//         const mm    = String(now.getMonth() + 1).padStart(2, "0");
//         const hh    = String(now.getHours()).padStart(2, "0");
//         return from2 + to2 + dd + mm + hh; // always exactly 10 chars
//     }
...
//             const tripCode = generateCode(form.from, form.to); // sync, always 10 chars
//             const now      = Date.now();
//             const data     = {
//                 tripCode,
//                 driverPhone: form.driverPhone,
...
//         const found = allCodes.find(tc => tc.tripCode === q);
...
//                         </h3>
//                         <div style={{ marginBottom: 14 }}>
//                             <label style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: .5, display: "block", marginBottom: 6 }}>Assign Driver *</label>
//                             <select style={INP} value={form.driverPhone} onChange={e => {
//                                 const d = drivers.find(x => x.phone === e.target.value);
//                                 const b = d?.busId ? buses.find(b => b.id === d.busId)?.number || "" : "";
//                                 setForm(f => ({ ...f, driverPhone: e.target.value, busNo: b }));
...
//                         </div>
//                         <div style={{ marginBottom: 14 }}>
//                             <label style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: .5, display: "block", marginBottom: 6 }}>Bus *</label>
//                             <select style={INP} value={form.busNo} onChange={e => setForm(f => ({ ...f, busNo: e.target.value }))}>
//                                 <option value="">— Select Bus —</option>
//                                 {buses.map(b => <option key={b.id} value={b.number}>{b.number} · {b.model}</option>)}
//                             </select>
//                         </div>
//                         <div style={{ marginBottom: 14 }}>
//                             <label style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: .5, display: "block", marginBottom: 6 }}>From District *</label>
//                             <select style={INP} value={form.from} onChange={e => setForm(f => ({ ...f, from: e.target.value }))}>
//                                 <option value="">— Select District —</option>
//                                 {TAMIL_DISTRICTS.map(d => <option key={d} value={d}>{d} ({DIST_CODE[d]})</option>)}
//                             </select>
//                         </div>
//                         <div style={{ marginBottom: 20 }}>
//                             <label style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: .5, display: "block", marginBottom: 6 }}>To District *</label>
//                             <select style={INP} value={form.to} onChange={e => setForm(f => ({ ...f, to: e.target.value }))}>
//                                 <option value="">— Select District —</option>
//                                 {TAMIL_DISTRICTS.filter(d => d !== form.from).map(d => <option key={d} value={d}>{d} ({DIST_CODE[d]})</option>)}
//                             </select>
//                         </div>
//                         {/* Live preview of what code will look like */}
//                         {form.from && form.to && (
//                             <div style={{ background: "#eef2ff", borderRadius: 12, padding: "12px 14px", marginBottom: 16, textAlign: "center" }}>
//                                 <div style={{ fontSize: 11, color: "#6366f1", fontWeight: 700, marginBottom: 6 }}>PREVIEW</div>
//                                 <div style={{ fontFamily: "monospace", fontSize: 22, fontWeight: 900, letterSpacing: 3, display: "flex", justifyContent: "center", gap: 2 }}>
//                                     <span style={{ color: "#4f46e5" }}>{DIST_CODE[form.from] || form.from.slice(0,2).toUpperCase()}</span>
//                                     <span style={{ color: "#10b981" }}>{DIST_CODE[form.to]   || form.to.slice(0,2).toUpperCase()}</span>
//                                     <span style={{ color: "#f59e0b" }}>{String(new Date().getDate()).padStart(2,"0")}</span>
//                                     <span style={{ color: "#ef4444" }}>{String(new Date().getMonth()+1).padStart(2,"0")}</span>
//                                     <span style={{ color: "#8b5cf6" }}>{String(new Date().getHours()).padStart(2,"0")}</span>
//                                 </div>
//                                 <div style={{ fontSize: 11, color: "#64748b", marginTop: 6, display: "flex", justifyContent: "center", gap: 10, flexWrap: "wrap" }}>
//                                     <span><span style={{ color: "#4f46e5", fontWeight: 700 }}>{DIST_CODE[form.from]}</span> = {form.from}</span>
//                                     <span><span style={{ color: "#10b981", fontWeight: 700 }}>{DIST_CODE[form.to]}</span> = {form.to}</span>
//                                     <span><span style={{ color: "#f59e0b", fontWeight: 700 }}>{String(new Date().getDate()).padStart(2,"0")}</span> = Date</span>
//                                     <span><span style={{ color: "#ef4444", fontWeight: 700 }}>{String(new Date().getMonth()+1).padStart(2,"0")}</span> = Month</span>
//                                     <span><span style={{ color: "#8b5cf6", fontWeight: 700 }}>{String(new Date().getHours()).padStart(2,"0")}</span> = Hour</span>
//                                 </div>
//                             </div>
//                         )}
//                         <button onClick={createTripCode} disabled={loading}
//                             style={{ width: "100%", background: loading ? "#94a3b8" : "#4f46e5", color: "#fff", border: "none", padding: "14px", fontSize: 15, fontWeight: 800, borderRadius: 12, cursor: loading ? "not-allowed" : "pointer" }}>
//                             {loading ? "⏳ Generating..." : "🎫 Generate Trip Code"}
//                         </button>
//                     </div>
...
//                     {generated && (
//                         <div style={{ background: "#f0fdf4", border: "2px solid #86efac", borderRadius: 20, padding: 24, textAlign: "center" }}>
//                             <div style={{ fontSize: 13, color: "#166534", fontWeight: 700, marginBottom: 10 }}>✅ Trip Code Generated!</div>
//                             {/* Big code display */}
//                             <div style={{ fontSize: 42, fontWeight: 900, color: "#15803d", letterSpacing: 6, fontFamily: "monospace", marginBottom: 8 }}>
//                                 {generated.tripCode}
//                             </div>
//                             {/* Code breakdown — color coded */}
//                             <div style={{ display: "flex", justifyContent: "center", gap: 4, marginBottom: 10, flexWrap: "wrap" }}>
//                                 {[
//                                     { chars: generated.tripCode.slice(0,2), label: "From",  color: "#4f46e5" },
//                                     { chars: generated.tripCode.slice(2,4), label: "To",    color: "#10b981" },
//                                     { chars: generated.tripCode.slice(4,6), label: "Date",  color: "#f59e0b" },
//                                     { chars: generated.tripCode.slice(6,8), label: "Month", color: "#ef4444" },
//                                     { chars: generated.tripCode.slice(8,10),label: "Hour",  color: "#8b5cf6" },
//                                 ].map(({ chars, label, color }) => (
//                                     <div key={label} style={{ textAlign: "center" }}>
//                                         <div style={{ background: color, color: "white", padding: "4px 10px", borderRadius: 8, fontSize: 15, fontWeight: 900, fontFamily: "monospace", letterSpacing: 2 }}>{chars}</div>
//                                         <div style={{ fontSize: 10, color, fontWeight: 700, marginTop: 3 }}>{label}</div>
//                                     </div>
//                                 ))}
//                             </div>
//                             <button onClick={() => copyCode(generated.tripCode)}
//                                 style={{ background: copied ? "#15803d" : "#4f46e5", color: "#fff", border: "none", padding: "9px 22px", borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: "pointer", marginBottom: 14 }}>
//                                 {copied ? "✅ Copied!" : "📋 Copy Code"}
//                             </button>
//                             <div style={{ fontSize: 13, color: "#374151", lineHeight: 1.8, textAlign: "left", background: "#fff", borderRadius: 12, padding: "12px 16px" }}>
...
//                         <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: "#0f172a" }}>📋 All Generated Trip Codes</h3>
//                         <span style={{ fontSize: 12, fontWeight: 700, background: "#eef2ff", color: "#4f46e5", padding: "4px 12px", borderRadius: 20 }}>{allCodes.length} total</span>
//                     </div>

//                     {/* Search bar */}
//                     <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
//                         <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8, border: "2px solid #e2e8f0", borderRadius: 12, paddingInline: 12, paddingBlock: 8, background: "#f8fafc" }}>
//                             <span style={{ fontSize: 16 }}>🔍</span>
//                             <input
//                                 style={{ flex: 1, border: "none", background: "transparent", fontSize: 14, fontWeight: 700, color: "#0f172a", letterSpacing: 1, outline: "none", fontFamily: "monospace" }}
//                                 placeholder="Search trip code... e.g. TVTD030314"
//                                 value={search}
//                                 maxLength={10}
//                                 onChange={e => { setSearch(e.target.value.toUpperCase()); if (!e.target.value) setSearchResult(null); }}
//                                 onKeyDown={e => e.key === "Enter" && searchCode()}
//                             />
//                             {search && (
//                                 <button onClick={clearSearch} style={{ border: "none", background: "none", cursor: "pointer", fontSize: 16, color: "#94a3b8", padding: 0 }}>✕</button>
//                             )}
//                         </div>
//                         <button onClick={searchCode} style={{ background: "#4f46e5", color: "#fff", border: "none", padding: "8px 18px", borderRadius: 12, fontWeight: 800, fontSize: 13, cursor: "pointer" }}>
//                             Search
//                         </button>
//                     </div>
...
//                     {/* Search Result */}
//                     {searchResult === false && (
//                         <div style={{ textAlign: "center", background: "#fef2f2", border: "1.5px solid #fca5a5", borderRadius: 14, padding: "24px 16px", marginBottom: 16 }}>
//                             <div style={{ fontSize: 32, marginBottom: 8 }}>❌</div>
//                             <div style={{ fontWeight: 800, color: "#dc2626", fontSize: 15 }}>Trip Code Not Found</div>
//                             <div style={{ fontSize: 13, color: "#ef4444", marginTop: 4 }}>"{search}" does not match any generated code.</div>
//                         </div>
//                     )}

//                     {searchResult && (
//                         <div style={{ background: "linear-gradient(135deg,#eef2ff,#f0fdf4)", border: "2px solid #818cf8", borderRadius: 16, padding: 20, marginBottom: 16 }}>
//                             <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
//                                 <div>
//                                     <div style={{ fontSize: 11, fontWeight: 700, color: "#6366f1", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>🔍 Search Result</div>
//                                     <div style={{ fontFamily: "monospace", fontSize: 28, fontWeight: 900, color: "#4f46e5", letterSpacing: 3 }}>{searchResult.tripCode}</div>
//                                 </div>
//                                 <span style={{ fontSize: 12, fontWeight: 800, background: statusBg[searchResult.status] || "#f3f4f6", color: statusColor[searchResult.status] || "#374151", padding: "6px 14px", borderRadius: 20, textTransform: "uppercase" }}>
//                                     {searchResult.status}
//                                 </span>
//                             </div>
//                             {/* Code breakdown */}
...
//                                 {[
//                                     { chars: searchResult.tripCode.slice(0,2), label: "From",  color: "#4f46e5" },
//                                     { chars: searchResult.tripCode.slice(2,4), label: "To",    color: "#10b981" },
//                                     { chars: searchResult.tripCode.slice(4,6), label: "Date",  color: "#f59e0b" },
//                                     { chars: searchResult.tripCode.slice(6,8), label: "Month", color: "#ef4444" },
//                                     { chars: searchResult.tripCode.slice(8,10),label: "Hour",  color: "#8b5cf6" },
//                                 ].map(({ chars, label, color }) => (
//                                     <div key={label} style={{ textAlign: "center" }}>
//                                         <div style={{ background: color, color: "white", padding: "3px 10px", borderRadius: 7, fontSize: 13, fontWeight: 900, fontFamily: "monospace" }}>{chars}</div>
...
//                                 {[
//                                     { icon: "👤", label: "Driver",      val: searchResult.driverName || "—" },
//                                     { icon: "📞", label: "Phone",       val: searchResult.driverPhone || "—" },
//                                     { icon: "🚌", label: "Bus No",      val: searchResult.busNo || "—" },
//                                     { icon: "📍", label: "From",        val: searchResult.from || "—" },
//                                     { icon: "🏁", label: "To",          val: searchResult.to || "—" },
//                                     { icon: "🕐", label: "Created",     val: searchResult.createdAt ? new Date(searchResult.createdAt).toLocaleString("en-IN", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "—" },
//                                     { icon: "✅", label: "Activated",   val: searchResult.activatedAt ? new Date(searchResult.activatedAt).toLocaleString("en-IN", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : "Not yet" },
//                                     { icon: "🔴", label: "Ended",       val: searchResult.endedAt ? new Date(searchResult.endedAt).toLocaleString("en-IN", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : "Not yet" },
//                                 ].map(({ icon, label, val }) => (
//                                     <div key={label} style={{ background: "rgba(255,255,255,0.7)", borderRadius: 10, padding: "10px 12px" }}>
//                                         <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 3 }}>{icon} {label}</div>
//                                         <div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a" }}>{val}</div>
//                                     </div>
//                                 ))}
//                             </div>
//                             <button onClick={() => copyCode(searchResult.tripCode)} style={{ marginTop: 14, width: "100%", background: copied ? "#15803d" : "#4f46e5", color: "#fff", border: "none", padding: "10px", borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: "pointer" }}>
//                                 {copied ? "✅ Copied!" : "📋 Copy Trip Code"}
//                             </button>
//                         </div>
//                     )}
...
//                         {allCodes.map(tc => (
//                             <div key={tc.tripCode}
//                                 onClick={() => { setSearch(tc.tripCode); setSearchResult(tc); }}
//                                 style={{ border: `1.5px solid ${searchResult && searchResult.tripCode === tc.tripCode ? "#818cf8" : "#e2e8f0"}`, borderRadius: 14, padding: "14px 16px", display: "grid", gridTemplateColumns: "160px 1fr auto", gap: 12, alignItems: "center", cursor: "pointer", background: searchResult && searchResult.tripCode === tc.tripCode ? "#eef2ff" : "white", transition: "all .15s" }}>
//                                 <div>
//                                     <div style={{ fontFamily: "monospace", fontSize: 18, fontWeight: 900, color: "#4f46e5", letterSpacing: 1 }}>{tc.tripCode}</div>
//                                     <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 3 }}>
//                                         {tc.createdAt ? new Date(tc.createdAt).toLocaleString("en-IN", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : "—"}
//                                     </div>
//                                 </div>
...
//                                     </span>
//                                     {tc.status === "pending" && (
//                                         <button onClick={e => { e.stopPropagation(); copyCode(tc.tripCode); }} style={{ background: "#eef2ff", border: "none", color: "#4f46e5", padding: "5px 12px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
//                                             📋 Copy
//                                         </button>
//                                     )}
//                                 </div>
...
function Badge({ val }) {
    const map = {
        ...
        completed: "#d1fae5,#065f46", "in-progress": "#dbeafe,#1e40af",
        scheduled: "#fef3c7,#92400e", cancelled: "#fee2e2,#991b1b", inactive: "#f3f4f6,#374151",
        pending: "#fef3c7,#92400e", verified: "#d1fae5,#065f46", rejected: "#fee2e2,#991b1b",
    };
    const [bg, color] = (map[val] || "#f3f4f6,#374151").split(",");
    return (
        <span style={{ background: bg, color, padding: "3px 12px", borderRadius: 20, fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: .5 }}>
...
// ═══════════════════════════════════════════════════════════
// SHARED HELPER: AUTOCOMPLETE INPUT
// ═══════════════════════════════════════════════════════════
        ...
        <div style={{ position: "relative", width: "100%" }}>
            ...
            {show && filtered.length > 0 && (
                <div style={{
                    ...
                    zIndex: 1000, marginTop: 5, maxHeight: 200, overflowY: "auto", border: "1px solid #e2e8f0"
                }}>
                    {filtered.map((s, idx) => (
                        <div key={idx} onMouseDown={() => { onChange(s); setShow(false); }} style={{
                            padding: "10px 14px", cursor: "pointer", fontSize: 13, borderBottom: "1px solid #f1f5f9"
...
function Dashboard({ buses, drivers, routes, trips, company, setPage }) {
    ...
    const activeBuses = buses.filter(b => b.status === "active").length;
    const onDuty = drivers.filter(d => d.status === "on-duty").length;
    const totalRev = trips.filter(t => t.status === "ended").reduce((s, t) => s + (t.revenue || 0), 0);
    ...
    return (
        <div>
            ...
            <div style={{
                ...
                minHeight: 130, display: "flex", alignItems: "center", marginBottom: 22,
                boxShadow: "0 4px 20px rgba(0,0,0,.08)",
            }}>
                <div style={{ padding: "24px 28px", flex: 1, zIndex: 2, cursor: "default" }}>
                    <div style={{ fontSize: 20, fontWeight: 800, color: "white" }}>
                        Welcome, {company.name}
                    </div>
                    <div style={{ fontSize: 13, color: "rgba(255,255,255,.8)", marginTop: 5 }}>
                        Company Admin Panel &nbsp;·&nbsp; {new Date().toDateString()}
                    </div>
                    <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
                        {[`${buses.length} Buses`, `${drivers.length} Drivers`, `${routes.length} Routes`].map(t => (
                            <span key={t} style={{
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 22 }}>
                {[
                    { emoji: "◈", label: "Total Buses", value: buses.length, sub: `${activeBuses} active` },
                    { emoji: "◉", label: "Drivers", value: drivers.length, sub: `${onDuty} on duty` },
                    { emoji: "◎", label: "Routes", value: routes.length, sub: `${routes.filter(r => r.status === "active").length} active` },
                    { emoji: "▤", label: "Today's Trips", value: todayTrips.length, sub: `${inProgress} in progress` },
                    { emoji: "₹", label: "Total Revenue", value: `₹${totalRev.toLocaleString()}`, sub: "All time" },
                ].map(s => (
                    <div key={s.label} style={{
                        background: "rgba(255, 255, 255, 0.85)", backdropFilter: "blur(8px)", borderRadius: 14, padding: "16px 20px",
                        boxShadow: "0 2px 10px rgba(0,0,0,.06)", flex: 1, minWidth: 140,
                    }}>
                        <div style={{ fontSize: 20, marginBottom: 6, opacity: 0.5 }}>{s.emoji}</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                {/* Today's trips */}
                <div style={{ background: "rgba(255, 255, 255, 0.85)", backdropFilter: "blur(8px)", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 10px rgba(0,0,0,.06)" }}>
                    <div style={{ padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f1f5f9" }}>
                        <span style={{ fontWeight: 700, fontSize: 14, color: "#0f172a" }}>Today's Trip Activity</span>
                        <button onClick={() => setPage("trips")} style={{ ...B.primary, fontSize: 12, padding: "5px 12px" }}>View All</button>
                    </div>
                    {todayTrips.length === 0 ? (
                        <div style={{ padding: 28, textAlign: "center", color: "#94a3b8", fontSize: 14 }}>No trips started today.</div>
                    ) : (
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                            ...
                            <tbody>
                                {todayTrips.map(t => (
                                    <tr key={t.trip_code} onMouseEnter={e => e.currentTarget.style.background = "#f8fafc"} onMouseLeave={e => e.currentTarget.style.background = ""}>
                                        <td style={TABLE.td}>{t.from_location} → {t.to_location}</td>
                                        <td style={TABLE.td}>{t.activated_at ? new Date(t.activated_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "Pending"}</td>
                                        <td style={TABLE.td}><Badge val={t.status} /></td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                <div style={{ background: "rgba(255, 255, 255, 0.85)", backdropFilter: "blur(8px)", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 10px rgba(0,0,0,.06)" }}>
                    <div style={{ padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f1f5f9" }}>
                        <span style={{ fontWeight: 700, fontSize: 14, color: "#0f172a" }}>Fleet Overview</span>
                        <button onClick={() => setPage("buses")} style={{ ...B.primary, fontSize: 12, padding: "5px 12px" }}>Manage</button>
                    </div>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                        <thead><tr style={{ background: "#f8fafc" }}>
                            <th style={TABLE.th}>Bus No.</th>
                            <th style={TABLE.th}>Driver</th>
                            <th style={TABLE.th}>Status</th>
                        </tr></thead>
                        <tbody>
                            {buses.map(b => (
                                <tr key={b.id} onMouseEnter={e => e.currentTarget.style.background = "#f8fafc"} onMouseLeave={e => e.currentTarget.style.background = ""}>
                                    <td style={TABLE.td}>{b.number}</td>
                                    <td style={TABLE.td}>{b.driver_name || drivers.find(d => d.id === b.driver_id)?.name || "—"}</td>
                                    <td style={TABLE.td}><Badge val={b.status} /></td>
                                </tr>
                            ))}
                        </tbody>
...
// 2. BUS MANAGEMENT
// ═══════════════════════════════════════════════════════════
function BusManagement({ buses, setBuses, drivers, routes, showToast, refreshData }) {
    const [modal, setModal] = useState(null);
    const [sel, setSel] = useState(null);
    const [form, setForm] = useState(EMPTY_BUS);
    const [search, setSearch] = useState("");
    ...
    const filtered = buses
        .filter(b => filter === "All" || (b.status || "active").toLowerCase() === filter.toLowerCase())
        .filter(b => (b.bus_number || b.number || "").toLowerCase().includes(search.toLowerCase()) || (b.model || "").toLowerCase().includes(search.toLowerCase()));

    const driverName = b => b.driver_name || "Unassigned";
    const routeName = b => (b.start_location && b.end_location) ? `${b.start_location} → ${b.end_location}` : "—";

    const save = async () => {
        if (!form.number) { showToast("Bus number is required.", "error"); return; }
        const token = localStorage.getItem("bus_token");
        try {
            const res = await fetch("http://localhost:5000/api/company/buses", {
                method: "POST",
                headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                body: JSON.stringify({ bus_number: form.number })
            });
    ...
    const deleteBus = async (b) => {
        const token = localStorage.getItem("bus_token");
        const busId = b.bus_id || b.id;
        try {
            const res = await fetch(`http://localhost:5000/api/company/buses/${busId}`, {
                method: "DELETE",
            ...
            if (data.success) {
                ...
                showToast(data.error || "Failed to delete bus.", "error");
            }
        } catch { showToast("Network error.", "error"); }
        setModal(null);
    };
    ...
    return (
        <div>
            ...
            <div style={TABLE.card}>
                <table style={TABLE.table}>
                    <thead><tr>
                        <th style={TABLE.th}>Bus No.</th><th style={TABLE.th}>Model</th><th style={TABLE.th}>Route</th>
                        <th style={TABLE.th}>Driver</th><th style={TABLE.th}>Status</th><th style={TABLE.th}>Actions</th>
                    </tr></thead>
                    <tbody>
                        {filtered.length === 0 && <tr><td colSpan={6} style={{ padding: 32, textAlign: "center", color: "#94a3b8" }}>No buses found.</td></tr>}
                        {filtered.map(b => (
                            <tr key={b.bus_id || b.id} onMouseEnter={e => e.currentTarget.style.background = "#f8fafc"} onMouseLeave={e => e.currentTarget.style.background = ""}>
                                <td style={TABLE.td}><code style={{ background: "#eef2ff", color: "#4f46e5", padding: "3px 8px", borderRadius: 6, fontSize: 12, fontWeight: 700 }}>{b.bus_number || b.number}</code></td>
                                <td style={{ ...TABLE.td, fontWeight: 600 }}>{b.model || "—"}</td>
                                <td style={TABLE.td}>{routeName(b)}</td>
                                <td style={TABLE.td}>{driverName(b)}</td>
                                <td style={TABLE.td}><Badge val={b.status || "active"} /></td>
                                <td style={TABLE.td}>
                                    <div style={{ display: "flex", gap: 5 }}>
                                        <button style={{ ...B.sm, background: "#fee2e2", color: "#991b1b" }} onClick={() => { setSel(b); setModal("delete") }}>🗑</button>
                                    </div>
            ...
            {modal === "add" && (
                <Modal title="🚌 Add New Bus" onClose={() => setModal(null)}>
                    <div style={{ marginBottom: 24 }}>
                        <Field label="Bus Number" required>
                            <input style={INP} value={form.number} onChange={e => setForm(f => ({ ...f, number: e.target.value }))} placeholder="e.g. TN-01-AB-1234" />
                        </Field>
                    </div>
                    <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                        <button style={B.cancel} onClick={() => setModal(null)}>Cancel</button>
                        <button style={B.primary} onClick={save}>Add Bus</button>
                    </div>
                </Modal>
            )}
            {modal === "delete" && sel && (
                <ConfirmDel name={sel.bus_number || sel.number} onConfirm={() => deleteBus(sel)} onClose={() => setModal(null)} />
            )}
        </div>
    );
}
...
// 3. DRIVER MANAGEMENT
// ═══════════════════════════════════════════════════════════
function DriverManagement({ drivers, setDrivers, buses, showToast, refreshData }) {
    const [modal, setModal] = useState(null);
    const [sel, setSel] = useState(null);
    const [form, setForm] = useState(EMPTY_DRIVER);
    const [search, setSearch] = useState("");
    ...
    const filtered = drivers
        .filter(d => filter === "All" || (d.status || "").toLowerCase().replace(" ", "-") === filter.toLowerCase().replace(" ", "-"))
        .filter(d => (d.name || "").toLowerCase().includes(search.toLowerCase()) || (d.phone || "").toLowerCase().includes(search.toLowerCase()));

    const busNum = d => d.bus_number || "Unassigned";
    ...
    const save = async () => {
        ...
        if (form.phone.length < 10) { showToast("Phone must be at least 10 digits.", "error"); return; }
        const generatedPwd = generatePassword(form.phone);
        const token = localStorage.getItem("bus_token");
        try {
            const res = await fetch("http://localhost:5000/api/company/drivers", {
                method: "POST",
                headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                body: JSON.stringify({
                    name: form.name, phone: form.phone, password: generatedPwd,
                    address: form.address || null,
                    licence_number: form.licence || null,
                    licence_expiry: form.licenceExpiry || null,
                    experience: form.experience || null,
                    police_verification: form.policeVerification || 'pending'
                })
            });
            const data = await res.json();
            if (data.success) {
                ...
                setCredentials({ name: form.name, phone: form.phone, password: generatedPwd });
            } else {
    ...
    const deleteDriver = async (d) => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/api/company/drivers/${driverId}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` }
            });
            const data = await res.json();
    ...
    const copyCredentials = () => {
        ...
        const text = `GoBus Driver Login\nPhone: ${credentials.phone}\nPassword: ${credentials.password}`;
        navigator.clipboard.writeText(text).then(() => showToast("Credentials copied!")).catch(() => { });
    };

    return (
        <div>
            <div style={PG.hdr}>
                <div><h2 style={PG.title}>Driver Management</h2><p style={PG.sub}>{drivers.length} drivers</p></div>
                <button style={B.primary} onClick={() => { setForm(EMPTY_DRIVER); setModal("add"); }}>+ Add Driver</button>
            </div>
            <div style={PG.bar}>
                <input style={PG.srch} placeholder="🔍 Search by name or phone..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            <div style={TABLE.card}>
                <table style={TABLE.table}>
                    <thead><tr>
                        <th style={TABLE.th}>Name</th><th style={TABLE.th}>Phone</th>
                        <th style={TABLE.th}>Licence No.</th><th style={TABLE.th}>Experience</th>
                        <th style={TABLE.th}>Police Verification</th><th style={TABLE.th}>Assigned Bus</th><th style={TABLE.th}>Actions</th>
                    </tr></thead>
                    <tbody>
                        {filtered.length === 0 && <tr><td colSpan={7} style={{ padding: 32, textAlign: "center", color: "#94a3b8" }}>No drivers found.</td></tr>}
                        {filtered.map(d => (
                            <tr key={d.driver_id || d.id} onMouseEnter={e => e.currentTarget.style.background = "#f8fafc"} onMouseLeave={e => e.currentTarget.style.background = ""}>
                                <td style={{ ...TABLE.td, fontWeight: 700 }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                        <div style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg,#4f46e5,#818cf8)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, flexShrink: 0 }}>{(d.name || "?")[0]}</div>
                                        {d.name}
                                ...
                                <td style={TABLE.td}>{d.phone}</td>
                                <td style={TABLE.td}><code style={{ background: "#f1f5f9", padding: "2px 7px", borderRadius: 5, fontSize: 12 }}>{d.licence_number || "—"}</code></td>
                                <td style={TABLE.td}>{d.experience || "—"}</td>
                                <td style={TABLE.td}><Badge val={d.police_verification || "pending"} /></td>
                                <td style={TABLE.td}>{busNum(d)}</td>
                                <td style={TABLE.td}>
                                    <div style={{ display: "flex", gap: 5 }}>
                                        <button style={{ ...B.sm, background: "#f1f5f9", color: "#475569" }} onClick={() => { setSel(d); setModal("view") }}>👁</button>
                                        <button style={{ ...B.sm, background: "#fee2e2", color: "#991b1b" }} onClick={() => { setSel(d); setModal("delete") }}>🗑</button>
            ...
            {modal === "add" && (
                <Modal title="👤 Add Driver" onClose={() => setModal(null)}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>
                        <Field label="Full Name" required>
                            <input style={INP} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Ramesh Kumar" />
                        </Field>
                        <Field label="Mobile Number (10 digits)" required>
                            <input style={INP} type="tel" maxLength={10} value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value.replace(/\D/g, "") }))} placeholder="e.g. 9876543210" />
                        </Field>
                        <Field label="Permanent Address" span>
                            <input style={INP} value={form.address || ""} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} placeholder="e.g. 12, MG Road, Chennai" />
                        </Field>
                        <Field label="Driving Licence Number">
                            <input style={INP} value={form.licence || ""} onChange={e => setForm(f => ({ ...f, licence: e.target.value }))} placeholder="e.g. TN-2023-12345" />
                        </Field>
                        <Field label="Licence Expiry Date">
                            <input style={INP} type="date" value={form.licenceExpiry || ""} onChange={e => setForm(f => ({ ...f, licenceExpiry: e.target.value }))} />
                        </Field>
                        <Field label="Total Driving Experience">
                            <input style={INP} value={form.experience || ""} onChange={e => setForm(f => ({ ...f, experience: e.target.value }))} placeholder="e.g. 5 years" />
                        </Field>
                        <Field label="Police Verification Status">
                            <select style={SEL} value={form.policeVerification || "pending"} onChange={e => setForm(f => ({ ...f, policeVerification: e.target.value }))}>
                    ...
                    {form.phone.length >= 10 && (
                        <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 10, padding: "10px 14px", marginBottom: 18, fontSize: 13, color: "#166534" }}>
                            🔑 Auto-generated password: <strong>{generatePassword(form.phone)}</strong>
                        </div>
                    )}
                    <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                        <button style={B.cancel} onClick={() => setModal(null)}>Cancel</button>
                        <button style={B.primary} onClick={save}>Add Driver</button>
                    </div>
                </Modal>
            )}
            {modal === "view" && sel && (
                <Modal title="👤 Driver Profile" onClose={() => setModal(null)}>
                    <div style={{ textAlign: "center", marginBottom: 20 }}>
                        <div style={{ width: 64, height: 64, borderRadius: "50%", background: "linear-gradient(135deg,#4f46e5,#8b5cf6)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, fontWeight: 800, margin: "0 auto 10px" }}>{(sel.name || "?")[0]}</div>
                        <div style={{ fontSize: 18, fontWeight: 800, color: "#0f172a" }}>{sel.name}</div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
                        {[["Phone", sel.phone], ["Password", sel.password], ["Licence No.", sel.licence_number || "—"], ["Licence Expiry", sel.licence_expiry || "—"], ["Experience", sel.experience || "—"], ["Address", sel.address || "—"], ["Police Verification", (sel.police_verification || "pending").toUpperCase()], ["Assigned Bus", busNum(sel)]].map(([k, v]) => (
                            <div key={k} style={{ background: "#f8fafc", padding: "11px 14px", borderRadius: 10 }}>
                                <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 700, textTransform: "uppercase" }}>{k}</div>
                                <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", marginTop: 3 }}>{v}</div>
                            </div>
            ...
            {modal === "credentials" && credentials && (
                <Modal title="✅ Driver Added Successfully" onClose={() => { setCredentials(null); setModal(null); }}>
                    <div style={{ textAlign: "center", marginBottom: 20 }}>
                        <div style={{ width: 64, height: 64, borderRadius: "50%", background: "linear-gradient(135deg,#10b981,#34d399)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, fontWeight: 800, margin: "0 auto 12px" }}>✓</div>
                        <div style={{ fontSize: 16, fontWeight: 700, color: "#0f172a" }}>{credentials.name}</div>
                        <div style={{ fontSize: 13, color: "#64748b", marginTop: 3 }}>has been added as a driver</div>
                    </div>
                    <div style={{ background: "#f8fafc", borderRadius: 14, padding: 20, border: "1.5px solid #e2e8f0", marginBottom: 20 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: "#4f46e5", textTransform: "uppercase", letterSpacing: 1, marginBottom: 14 }}>📱 GoBus App Login Credentials</div>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                            <div style={{ background: "white", padding: "12px 14px", borderRadius: 10, border: "1px solid #e2e8f0" }}>
                                <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 700, textTransform: "uppercase" }}>Phone (Username)</div>
                                <div style={{ fontSize: 18, fontWeight: 800, color: "#0f172a", marginTop: 4, fontFamily: "monospace" }}>{credentials.phone}</div>
                            </div>
                            <div style={{ background: "white", padding: "12px 14px", borderRadius: 10, border: "1px solid #e2e8f0" }}>
                                <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 700, textTransform: "uppercase" }}>Password</div>
                                <div style={{ fontSize: 18, fontWeight: 800, color: "#0f172a", marginTop: 4, fontFamily: "monospace" }}>{credentials.password}</div>
                            </div>
                        </div>
                        <div style={{ marginTop: 14, fontSize: 12, color: "#64748b", background: "#fffbeb", padding: "8px 12px", borderRadius: 8, border: "1px solid #fde68a" }}>
                            ⚠️ Share these credentials with the driver. They will use these to log in to the GoBus Driver App.
                        </div>
                    </div>
                    <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                        <button style={{ ...B.primary, background: "#10b981" }} onClick={copyCredentials}>📋 Copy Credentials</button>
                        <button style={B.primary} onClick={() => { setCredentials(null); setModal(null); }}>Done</button>
                    </div>
                </Modal>
            )}
            {modal === "delete" && sel && (
                <ConfirmDel name={sel.name} onConfirm={() => deleteDriver(sel)} onClose={() => setModal(null)} />
            )}
        </div>
    );
}
...
// ═══════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════
// 5. TRIP MANAGEMENT — Real-time monitoring from Backend
// ═══════════════════════════════════════════════════════════
function TripManagement({ showToast, user, trips, fetchTrips, routes }) {
    const [filter, setFilter] = useState("All");
    const [search, setSearch] = useState("");
    ...
    const statusMap = {
        ...
        ended: { color: "#6b7280", bg: "#f3f4f6", label: "Ended" },
    ...
    return (
        <div>
            <div style={PG.hdr}>
                <div>
                    <h2 style={PG.title}>Real-time Trips</h2>
                    <p style={PG.sub}>Monitoring {trips.filter(t => t.status === 'active').length} active trips across the fleet</p>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                    <button onClick={fetchTrips} style={{ ...B.sm, background: "#fff", border: "1px solid #e2e8f0" }}>🔄 Refresh</button>
                </div>
            </div>

            <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
                {["All", "Active", "Pending", "Ended"].map(s => {
                    const count = s === "All" ? trips.length : trips.filter(t => t.status.toLowerCase() === s.toLowerCase()).length;
                    ...
                    return (
                        <div key={s} onClick={() => setFilter(s)} style={{
                            ...
                            boxShadow: "0 4px 12px rgba(0,0,0,.04)", transition: "all .2s",
                            display: "flex", alignItems: "center", gap: 10, border: "2px solid",
                            borderColor: isActive ? "#4f46e5" : "transparent"
                        }}>
                            <span style={{ fontWeight: 800, fontSize: 16 }}>{count}</span>
                            <span style={{ fontSize: 13, fontWeight: 700 }}>{s} Trips</span>
                        </div>
                    )
                })}
            </div>
            ...
            <div style={PG.bar}>
                <input style={PG.srch} placeholder="🔍 Search by Trip Code, Driver, or Bus..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            ...
            <div style={TABLE.card}>
                <table style={TABLE.table}>
                    <thead>
                        <tr>
                            <th style={TABLE.th}>Trip Code</th>
                            <th style={TABLE.th}>Route (From → To)</th>
                            <th style={TABLE.th}>Bus</th>
                            <th style={TABLE.th}>Driver</th>
                            <th style={TABLE.th}>Started At</th>
                            <th style={TABLE.th}>Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        {trips.length === 0 ? (
                            <tr><td colSpan={6} style={{ padding: 40, textAlign: "center", color: "#64748b" }}>Loading or no trips found...</td></tr>
                        ) : filtered.length === 0 ? (
                            <tr><td colSpan={6} style={{ padding: 40, textAlign: "center", color: "#64748b" }}>No matching trips found.</td></tr>
                        ) : filtered.map(t => {
                            const st = statusMap[t.status] || { color: "#64748b", bg: "#f1f5f9", label: t.status };
                            return (
                                <tr key={t.trip_code} style={{ borderBottom: "1px solid #f1f5f9" }}>
                                    <td style={TABLE.td}>
                                        <code style={{ background: "#eef2ff", color: "#4f46e5", padding: "4px 8px", borderRadius: 8, fontSize: 13, fontWeight: 800 }}>{t.trip_code}</code>
                                    </td>
                                    <td style={TABLE.td}>
                                        <div style={{ fontWeight: 700, color: "#0f172a" }}>{t.from_location} → {t.to_location}</div>
                                        <div style={{ fontSize: 11, color: "#94a3b8" }}>Route ID: {t.route_id || "Direct"}</div>
                                    </td>
                                    <td style={TABLE.td}>
                                        <div style={{ fontWeight: 700 }}>{t.bus_number}</div>
                                    </td>
                                    <td style={TABLE.td}>
                                        <div style={{ fontWeight: 700 }}>{t.driver_name}</div>
                                        <div style={{ fontSize: 11, color: "#64748b" }}>{t.driver_phone}</div>
                                    </td>
                                    <td style={TABLE.td}>
                                        {t.activated_at ? new Date(t.activated_at).toLocaleString('en-IN', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' }) : "—"}
                                    </td>
                                    <td style={TABLE.td}>
                                        <span style={{
...
// ═══════════════════════════════════════════════════════════
// 6. LIVE TRACKING — with From/To search bar
// ═══════════════════════════════════════════════════════════
function LiveTracking({ buses, drivers }) {
    const [liveBuses, setLiveBuses] = useState([]);
    const [sel, setSel] = useState(null);
    ...
    // Use TN_DISTRICTS instead of parsing from routes
    ...
    // Filter by From/To search
    const displayBuses = liveBuses.filter(g => {
        const fromOk = !fromSearch || g.from.toLowerCase().includes(fromSearch.toLowerCase());
        const toOk = !toSearch || g.to.toLowerCase().includes(toSearch.toLowerCase());
        return fromOk && toOk;
    });

    const clearSearch = () => { setFromSearch(""); setToSearch(""); };
    ...
    return (
        <div>
            ...
            {/* Search bar — From / To */}
            <div style={{ background: "white", borderRadius: 16, padding: 18, marginBottom: 18, boxShadow: "0 4px 16px rgba(0,0,0,.06)" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#64748b", marginBottom: 12 }}>🔍 Search Buses by Route</div>
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
                    <div style={{ flex: 1, minWidth: 180 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 4 }}>FROM</div>
                        <AutocompleteInput
                    ...
                    <div style={{ fontSize: 20, color: "#94a3b8", marginTop: 18 }}>→</div>
                    <div style={{ flex: 1, minWidth: 180 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 4 }}>TO</div>
                        <AutocompleteInput
                            style={{ ...INP, background: "#f8fafc" }}
                            placeholder="e.g. Trichy, Airport..."
                            value={toSearch}
                    ...
                    {(fromSearch || toSearch) && (
                        <button onClick={clearSearch} style={{ ...B.sm, background: "#fee2e2", color: "#991b1b", marginTop: 18 }}>✕ Clear</button>
                    )}
                    <div style={{ marginTop: 18, background: "#eef2ff", borderRadius: 10, padding: "10px 16px", fontSize: 13, color: "#4f46e5", fontWeight: 700 }}>
                        {displayBuses.length} bus{displayBuses.length !== 1 ? "es" : ""} found
                    </div>
                </div>
                {(fromSearch || toSearch) && liveBuses.length > 0 && displayBuses.length === 0 && (
                    <div style={{ marginTop: 12, background: "#fff7ed", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "#92400e" }}>
                        ⚠️ No buses found for this route. Try different search terms.
                    </div>
                )}
            </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20 }}>
                    <div style={{ background: "#1e293b", borderRadius: 18, overflow: "hidden", position: "relative", minHeight: 480 }}>
                        ...
                        {displayBuses.map((g) => {
                            ...
                            const yClamped = Math.max(10, Math.min(85, 50 - (g.lat - baseLat) * 800));
                            const isSelected = sel === g.tripCode;
                            const isFiltered = (fromSearch || toSearch) && displayBuses.includes(g);
                            return (
                                <div key={g.tripCode} onClick={() => setSel(isSelected ? null : g.tripCode)}
                                    style={{ position: "absolute", left: `${xClamped}%`, top: `${yClamped}%`, cursor: "pointer", transform: "translate(-50%,-100%)", zIndex: isSelected ? 10 : 5, transition: "left 1s ease, top 1s ease" }}>
                                    <div style={{ background: isSelected ? "#f59e0b" : isFiltered ? "#10b981" : "#4f46e5", color: "white", borderRadius: "50% 50% 50% 0", width: 42, height: 42, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, boxShadow: `0 4px 20px ${isSelected ? "rgba(245,158,11,.6)" : "rgba(79,70,229,.5)"}`, border: `3px solid ${isSelected ? "#fde68a" : "white"}`, transform: "rotate(-45deg)" }}>
                                        <span style={{ transform: "rotate(45deg)" }}>🚌</span>
                                    </div>
                                    <div style={{ textAlign: "center", marginTop: 4, background: "rgba(15,23,42,.85)", color: "white", padding: "3px 8px", borderRadius: 6, fontSize: 11, fontWeight: 700, whiteSpace: "nowrap" }}>
                                        {g.busNo.slice(-8)}
                        ...
                        {displayBuses.length < liveBuses.length && (
                            <div style={{ position: "absolute", bottom: 16, right: 16, background: "rgba(245,158,11,.9)", color: "white", padding: "5px 12px", borderRadius: 8, fontSize: 11, fontWeight: 700 }}>
                                🔍 Filtered: {displayBuses.length}/{liveBuses.length} buses
                            </div>
                        )}
                        {displayBuses.length === liveBuses.length && (
                            <div style={{ position: "absolute", bottom: 16, right: 16, background: "rgba(16,185,129,.25)", color: "#10b981", padding: "5px 10px", borderRadius: 8, fontSize: 11, fontWeight: 700 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 12, maxHeight: 520, overflowY: "auto" }}>
                        {displayBuses.length === 0 && liveBuses.length > 0 && (
                            <div style={{ background: "white", borderRadius: 14, padding: 24, textAlign: "center", color: "#94a3b8" }}>
                                <div style={{ fontSize: 32, marginBottom: 8 }}>🔍</div>
                                <div style={{ fontSize: 13 }}>No buses match your search</div>
                            </div>
                        )}
                        {displayBuses.map(g => (
                            <div key={g.tripCode} onClick={() => setSel(sel === g.tripCode ? null : g.tripCode)}
...
// TripCodeManager is now powered by the MySQL backend — imported from TripCodeManager.js


// Legacy Firebase-based TripCodeManager removed.
...
// ═══════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════
// 8. REPORTS — Creative redesign
// ═══════════════════════════════════════════════════════════
function Reports({ trips, buses, drivers, routes }) {
    const completed = trips.filter(t => t.status === "ended");
    const inProgress = trips.filter(t => t.status === "active");
    const totalRev = completed.reduce((s, t) => s + (t.revenue || 0), 0);
    const totalPax = completed.reduce((s, t) => s + (t.passengers || 0), 0);

    return (
        <div>
            <div style={PG.hdr}>
                <div>
                    <h2 style={PG.title}>📊 Reports</h2>
                    <p style={PG.sub}>{completed.length} completed trips · ₹{totalRev.toLocaleString()} total revenue</p>
                </div>
                {inProgress.length > 0 && (
                    <span style={{ background: "#dbeafe", color: "#1e40af", padding: "6px 14px", borderRadius: 10, fontSize: 13, fontWeight: 700 }}>
                        🔵 {inProgress.length} trip{inProgress.length > 1 ? "s" : ""} in progress
                    </span>
                )}
            </div>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 24 }}>
                {[
                    { emoji: "✅", label: "Completed Trips", value: completed.length },
                    { emoji: "👥", label: "Total Passengers", value: totalPax.toLocaleString() },
                    { emoji: "💰", label: "Total Revenue", value: `₹${totalRev.toLocaleString()}` },
                    { emoji: "📊", label: "Avg Revenue/Trip", value: completed.length ? `₹${Math.round(totalRev / completed.length)}` : "—" },
                    { emoji: "🚌", label: "Buses Active", value: `${buses.filter(b => b.status === "active").length}/${buses.length}` },
                    { emoji: "🗺", label: "Routes Running", value: `${routes.filter(r => r.status === "active").length}/${routes.length}` },
                ].map(s => (
                    <div key={s.label} style={{
            ...
            <div style={TABLE.card}>
                <div style={{ padding: "16px 20px", borderBottom: "1px solid #f1f5f9" }}>
                    <span style={{ fontWeight: 700, fontSize: 14, color: "#0f172a" }}>📋 Trip Log</span>
                    <span style={{ fontSize: 12, color: "#94a3b8", marginLeft: 10 }}>{trips.length} records</span>
                </div>
                <div style={{ overflowX: "auto" }}>
                    <table style={TABLE.table}>
                        <thead><tr>
                            <th style={TABLE.th}>Trip ID</th>
                            <th style={TABLE.th}>Route</th>
                            <th style={TABLE.th}>Date</th>
                            <th style={TABLE.th}>Driver</th>
                            <th style={TABLE.th}>Bus</th>
                            <th style={TABLE.th}>Passengers</th>
                            <th style={TABLE.th}>Revenue</th>
                            <th style={TABLE.th}>Status</th>
                        </tr></thead>
                        <tbody>
                            {trips.length === 0 && (
                                <tr><td colSpan={8} style={{ padding: 28, textAlign: "center", color: "#94a3b8" }}>No trips recorded yet.</td></tr>
                            )}
                            {trips.map(t => (
                                <tr key={t.trip_code} onMouseEnter={e => e.currentTarget.style.background = "#f8fafc"} onMouseLeave={e => e.currentTarget.style.background = ""}>
                                    <td style={TABLE.td}><code style={{ background: "#eef2ff", color: "#4f46e5", padding: "2px 8px", borderRadius: 6, fontSize: 12 }}>{t.trip_code}</code></td>
                                    <td style={TABLE.td}>{t.from_location} → {t.to_location}</td>
                                    <td style={TABLE.td}>{t.created_at ? new Date(t.created_at).toLocaleDateString() : "—"}</td>
                                    <td style={TABLE.td}>{t.driver_name || "—"}</td>
                                    <td style={TABLE.td}>{t.bus_number || "—"}</td>
                                    <td style={TABLE.td}>{t.passengers || "—"}</td>
                                    <td style={TABLE.td}><strong style={{ color: t.revenue ? "#10b981" : "#94a3b8" }}>{t.revenue ? `₹${t.revenue}` : "—"}</strong></td>
                                    <td style={TABLE.td}><Badge val={t.status} /></td>
                                </tr>
                            ))}
                        </tbody>
...
// MAIN: COMPANY ADMIN
// ═══════════════════════════════════════════════════════════
export default function CompanyAdmin({ user, onLogout }) {
    const [page, setPage] = useState("dashboard");
    ...
    // Helper: detect expired/invalid token and auto-logout
    ...
    // Fetch drivers & buses from backend (single source of truth)
    const refreshData = () => {
        const token = localStorage.getItem("bus_token");
        if (!token) { onLogout(); return; }
        const headers = { Authorization: `Bearer ${token}` };
        fetch("http://localhost:5000/api/company/drivers", { headers })
            .then(r => { if (handleTokenError(r)) return null; return r.json(); })
            .then(d => { if (d && d.success) setDrivers(d.drivers); }).catch(() => { });
        fetch("http://localhost:5000/api/company/buses", { headers })
            .then(r => { if (handleTokenError(r)) return null; return r.json(); })
            .then(d => { if (d && d.success) setBuses(d.buses); }).catch(() => { });
    };
    ...
    const fetchTrips = async () => {
        try {
            const token = localStorage.getItem("bus_token");
            if (!token) return;
            const res = await fetch("http://localhost:5000/api/tripcodes", {
                headers: { "Authorization": `Bearer ${token}` }
            });
            if (handleTokenError(res)) return;
            const data = await res.json();
            if (data.success) setTrips(data.tripCodes);
        } catch (err) { console.error("Root trip fetch error:", err); }
    };
    ...
    const renderPage = () => {
        switch (page) {
            case "dashboard": return <Dashboard buses={buses} drivers={drivers} routes={routes} trips={trips} company={company} setPage={setPage} />;
            case "buses": return <BusManagement buses={buses} setBuses={setBuses} drivers={drivers} routes={routes} showToast={showToast} refreshData={refreshData} />;
            case "drivers": return <DriverManagement drivers={drivers} setDrivers={setDrivers} buses={buses} showToast={showToast} refreshData={refreshData} />;

            case "trips": return <TripManagement showToast={showToast} user={user} trips={trips} fetchTrips={fetchTrips} routes={routes} />;
            case "tripcodes": return <TripCodeManagerBackend drivers={drivers} buses={buses} user={user} routes={routes} />;
            case "tracking": return <LiveTracking buses={buses} drivers={drivers} />;
            case "reports": return <Reports trips={trips} buses={buses} drivers={drivers} routes={routes} />;
            case "profile": return <CompanyProfile company={company} setCompany={setCompany} showToast={showToast} />;
            default: return <Dashboard buses={buses} drivers={drivers} routes={routes} trips={trips} company={company} setPage={setPage} />;
    ...
    return (
        <div style={{ display: "flex", height: "100vh", fontFamily: "'Segoe UI',Tahoma,Geneva,Verdana,sans-serif", background: "transparent" }}>
            <style>{`
        @keyframes caFade { from{opacity:0;transform:translateY(-10px)} to{opacity:1;transform:translateY(0)} }
        @keyframes pulse  { 0%,100%{opacity:1} 50%{opacity:.4} }
        * { box-sizing: border-box; }
      ...
      `}</style>
            ...
            {/* ── SIDEBAR ── */}
            <aside style={{ width: sidebarOpen ? 240 : 72, background: "rgba(79, 70, 229, 0.8)", backdropFilter: "blur(12px)", color: "white", padding: "24px 0", display: "flex", flexDirection: "column", transition: "width .25s", overflow: "hidden", flexShrink: 0, position: "relative", zIndex: 10 }}>
                <div style={{ padding: "0 20px 28px", borderBottom: "1px solid rgba(255,255,255,.15)", marginBottom: 16, cursor: "default" }}>
                    <div style={{ fontSize: 20, fontWeight: 800, whiteSpace: "nowrap", overflow: "hidden", cursor: "default", letterSpacing: -0.3 }}>
                        {sidebarOpen ? "GoBus Admin" : "GB"}
                    </div>
                    {sidebarOpen && <div style={{ fontSize: 11, color: "rgba(255,255,255,.7)", marginTop: 3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{company.name}</div>}
                </div>
                ...
                <nav style={{ flex: 1, padding: "0 10px", display: "flex", flexDirection: "column", gap: 4 }}>
                    {NAV.map(n => (
                        <button key={n.key} onClick={() => setPage(n.key)} style={{
                            ...
                            background: page === n.key ? "rgba(255,255,255,.2)" : "transparent",
                            color: "white", border: "none", cursor: "pointer", textAlign: "left", whiteSpace: "nowrap",
                            fontWeight: page === n.key ? 700 : 400, fontSize: 13, width: "100%",
                            transition: "all .2s",
                            transform: page === n.key ? "translateX(4px)" : "none",
                            boxShadow: page === n.key ? "0 2px 8px rgba(0,0,0,.15)" : "none",
                        }}
                            onMouseEnter={e => { if (page !== n.key) { e.currentTarget.style.background = "rgba(255,255,255,.1)"; e.currentTarget.style.transform = "translateX(4px)"; } }}
                            onMouseLeave={e => { if (page !== n.key) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.transform = "none"; } }}>
                            <span style={{ fontSize: 18, flexShrink: 0 }}>{n.icon}</span>
                            {sidebarOpen && n.label}
                            {sidebarOpen && n.key === "tracking" && liveCount > 0 && (
                                <span style={{ marginLeft: "auto", background: "#ef4444", borderRadius: 20, fontSize: 10, fontWeight: 800, padding: "1px 7px" }}>{liveCount}</span>
                ...
                {sidebarOpen && (
                    <div style={{ padding: "16px 20px", borderTop: "1px solid rgba(255,255,255,.15)", marginTop: 8 }}>
                        <button onClick={onLogout} style={{
                            ...
                            color: "white", padding: "10px 16px", borderRadius: 10, cursor: "pointer",
                            width: "100%", fontSize: 13, fontWeight: 600, transition: "all .2s",
                        }}
            ...
            {/* ── MAIN ── */}
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                <header style={{ background: "rgba(255, 255, 255, 0.75)", backdropFilter: "blur(12px)", padding: "0 28px", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 2px 10px rgba(0,0,0,.06)", flexShrink: 0 }}>
                    <div style={{ cursor: "default" }}>
                        <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#0f172a", cursor: "default" }}>{pageTitle}</h1>
                        <p style={{ margin: 0, fontSize: 12, color: "#94a3b8", cursor: "default" }}>Company Admin · {company.name}</p>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(16,185,129,.1)", padding: "6px 12px", borderRadius: 20, border: "1px solid rgba(16,185,129,.2)" }}>
                            <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#10b981", animation: "pulse 1.5s infinite" }} />
                            <span style={{ fontSize: 12, fontWeight: 700, color: "#065f46" }}>{liveCount} Live</span>
                        </div>
                        <div style={{ position: "relative" }}>
                            <div style={{ background: "rgba(241,245,249,0.8)", width: 38, height: 38, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "background .2s" }}
                        ...
                        <div style={{ background: "rgba(79,70,229,0.1)", color: "#4f46e5", padding: "8px 16px", borderRadius: 10, fontWeight: 700, fontSize: 13, border: "1px solid rgba(79,70,229,.15)", cursor: "default" }}>
                            {company.name.split(" ")[0]}
                        </div>
                    </div>
                </header>

                <main style={{ flex: 1, overflowY: "auto", padding: 28, background: "rgba(240, 244, 248, 0.5)" }}>
                    {renderPage()}
                </main>
            </div>
        </div>
